...
- `gpu-buf` variance on AMD Hawaii PRO (75-102s, same config) does not reproduce on GTX 680 (flat to the ms, confirmed independently on kale's own GTX 680 too). **Root cause (re-diagnosed 2026-09-01, superseding the buffer-demotion theory below): Hawaii's memory-clock DVFS periodically drops `mclk` from 1500 MHz to 150 MHz (10x) during sustained `fp_buffer` execution.** Confirmed directly: a clock-probe instrumented run (`FP_BUFFER_CLOCK_PROBE=1`, `src/ct_gpu.c`) showed every slow slab sampling `mclk=150Mhz` and every fast baseline sampling `mclk=1500Mhz`, zero overlap across 30 epochs and ~35 slow-slab events, with `sclk` and temperature both staying flat throughout (rules out core-clock throttling and thermal throttling as the trigger). `gpu-img`/`gpu-opt` stayed perfectly flat in the same sessions gpu-buf degraded — expected, since they read through the texture cache and are far less sensitive to `mclk` state, while `fp_buffer`'s uncoalesced buffer access is bandwidth-bound. A repeat-slab run (60 fixed-slab repeats, no reallocation at any point) showed the same slab oscillating between the fast and slow cost tier *on its own*, multiple times, unprompted by any buffer free/recreate — this is what falsifies the earlier "step degradation that holds until `d_vol` is freed and recreated" theory below. `power_dpm_force_performance_level` (the sysfs knob to pin the GPU at its top DPM state) is present but not writable without root on this machine, so the mechanism can't be force-disabled at the driver level. **Mitigation attempted, DID NOT HOLD AT SCALE:** `FP_BUFFER_SKIP_SLAB_FINISH=1` (removes the idle gap between angle-slab launches) measured ~29% faster over a 30-epoch run (20.60s/epoch vs 29.14s/epoch unmitigated) and appeared to reduce both burst frequency and severity — but a subsequent **100-epoch** run measured **40.8s/epoch (4082.32s total)**, i.e. *worse* than the unmitigated baseline. The 30-epoch result was a short-run artifact. Per-epoch times in the 100-epoch run oscillate violently from the first epoch (47.9, 69.0, 15.8, … 66.9 … 35.5, 15.7, 50.8 s) rather than degrading progressively, and the 15.7-15.8 s floor still appears late in the run — the same bimodal DVFS signature, just landing in the slow state far more often. Note the flag also disables the per-slab SLOW diagnostic (it removes the `clFinish` those prints depend on), so that run reports zero slow-slab warnings despite obviously bursting; the absence of warnings there is an artifact, not evidence. Contention from another user on this shared machine is not excluded as a contributing factor. **This is the second mitigation to look good at short scale and fail at real scale** (see `FP_BUFFER_VOL_REALLOC_EVERY` below) — do not adopt either without a 100-epoch measurement. Neither eliminates the bursts — some slabs still degrade even fully packed. Recommended for real runs; leave unset only when actively re-diagnosing (it also disables the per-slab SLOW/HOST-SIDE-GAP diagnostic print). A follow-up attempt, `FP_BUFFER_ANG_SLAB=16` (fewer/larger angle-slabs on top of the above) was tested and made things *worse*, not better — 23.16s/epoch mean, ~12% slower, with the max epoch nearly doubling (63.9s vs 36.1s) — larger slabs let a burst's cost compound further before any chance of recovery. Keep `ANG_SLAB` at its default (8); `SKIP_SLAB_FINISH` alone is the recommended setting (see `src/ct_gpu.c` and `docs/performance-history.md` for the full numbers). Prefer `gpu-img`/`gpu-opt` over `gpu-buf` on this hardware when speed matters more than `gpu-buf`'s tighter CPU-parity margin.
