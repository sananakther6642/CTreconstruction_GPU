...
)
{
    ...
    /* Fast AABB hit test: skip rays that miss the volume entirely.
     * Slab method on world-space box [-hxz,hxz]^2 x [-hy,hy].
     * Avoids n_samples wasted iterations on edge/corner detector pixels. */
    ...
    float val = 0.f;
    if (tmin < tmax) {
        /* Clamp ray segment to AABB — skip empty samples at both ends */
        int s_start = (int)((tmin - near_t) / dt);
        int s_end   = (int)((tmax - near_t) / dt) + 1;
